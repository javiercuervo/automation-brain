...
| **N09b** | Diploma OCH — diseño corporativo | ✅ | Genera diplomas con branding IITD completo (fuente Quasimoda, colores de marca, logo, fondo decorativo) + QR + hash + firma digital + upload SiteGround | 1. `node diploma-och.mjs --mock` (datos ejemplo)<br>2. `node diploma-och.mjs --email alumno@email.com --upload`<br>3. Verificar PDF: logo, fuente Quasimoda, colores corporativos | `/integraciones/alumnos/generacion/diploma-och.mjs`<br>`/activos/fuentes/Quasimoda-*.otf`<br>`/activos/logo-fondo-transparente.png` |
